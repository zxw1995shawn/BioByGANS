...
def matrix_generation(sentence, ntokens, max_seq_length):
    ...
    for i, item in enumerate(ntokens):
        if item == "[CLS]":
            syntactic_feature_matrix[i][pos_map["CLS"]] = 1.0
        elif item == "[SEP]":
            syntactic_feature_matrix[i][pos_map["SEP"]] = 1.0
        elif item[0:2] == "##":
            syntactic_feature_matrix[i][pos_map["X"]] = 1.0
        elif item == "[PAD]":
            pass
        else:
            syntactic_feature_matrix[i][pos_map[pos[tem]]] = 1.0
            tem += 1
    ...
    for tail, head, rel in dependency:
        ...
        try:
            ...
            syntactic_feature_matrix[word_index[tail]][len(pos_list)+2*dependency_map[rel]] = 1.0
            syntactic_feature_matrix[word_index[head]][len(pos_list)+2*dependency_map[rel]+1] = 1.0
        except:
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    ...
    bias_matrix, syntactic_feature_matrix = matrix_generation(sentence, ntokens, max_seq_length)
...
def create_model(bert_config, is_training, input_ids, input_mask,
                 ...
                 bias_matrix, syntactic_feature_matrix,
                 dropout_rate=1.0, lstm_size=128, cell='lstm', num_layers=1):
