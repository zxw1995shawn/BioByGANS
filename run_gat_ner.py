...
import collections
import os
from lstm_gat_crf_layer import GAT_BILSTM_CRF
import modeling
import optimization
import tokenization
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_ids, ntokens, label_ids_word_level, 
        adjacent_matrix, syntactic_feature_matrix, trans_matrix):
        self.input_ids = input_ids
        self.input_mask = input_mask
        self.segment_ids = segment_ids
        self.label_ids = label_ids
        self.ntokens = ntokens
        self.label_ids_word_level = label_ids_word_level
...
def trans_matrix_generation(ntokens, max_seq_length):
    trans_matrix = np.zeros([max_seq_length, max_seq_length], dtype = np.float64)

    return trans_matrix

def matrix_generation(sentence, max_seq_length):
    adjacent_matrix = np.zeros((max_seq_length, max_seq_length), dtype = np.int32)
    syntactic_feature_matrix = np.zeros((max_seq_length, len(pos_list)+2*len(dependency_list)), dtype = np.float32)
    ...
    for i, item in enumerate(pos):
        syntactic_feature_matrix[i][pos_map[item]] = 1
    for tail, head, rel in dependency:
        rel = rel.lower()
        adjacent_matrix[tail][head] = 1
        adjacent_matrix[head][tail] = 1
        syntactic_feature_matrix[tail][len(pos_list)+2*dependency_map[rel]] = 1.0
        syntactic_feature_matrix[head][len(pos_list)+2*dependency_map[rel]+1] = 1.0
    ...
    return adjacent_matrix, syntactic_feature_matrix
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    ...
    adjacent_matrix, syntactic_feature_matrix = matrix_generation(sentence, max_seq_length)
    label_ids_word_level = []
    for i in labellist:
        label_ids_word_level.append(label_map[i])
    while len(label_ids_word_level) < max_seq_length:
        label_ids_word_level.append(0)
    assert len(label_ids_word_level) == max_seq_length

    trans_matrix = trans_matrix_generation(ntokens, max_seq_length)
    ...
    if ex_index < 4 : # Examples before model run
        ...
        tf.logging.info("label_ids_word_level: %s" % " ".join([str(x) for x in label_ids_word_level]))
        tf.logging.info("ntokens: %s" % " ".join([str(x) for x in ntokens]))
        tf.logging.info("adjacent_matrix: %s" % " ".join([str(x) for x in adjacent_matrix]))
    ...
    feature = InputFeatures(
        ...
        label_ids=label_ids,
        ntokens=ntokens,
        label_ids_word_level=label_ids_word_level,
...
def filed_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features["label_ids_word_level"] = create_int_feature(feature.label_ids_word_level)
        features["adjacent_matrix"] = create_float_feature(feature.adjacent_matrix)
        features["syntactic_feature_matrix"] = create_float_feature(feature.syntactic_feature_matrix)
        features["trans_matrix"] = create_float_feature(feature.trans_matrix)
...
def file_based_input_fn_builder(input_file, seq_length, is_training, drop_remainder):
    name_to_features = {
        ...
        "label_ids_word_level": tf.FixedLenFeature([seq_length], tf.int64),
        "ntokens": tf.FixedLenFeature([seq_length], tf.string),
        "adjacent_matrix": tf.VarLenFeature(tf.float32),
        "syntactic_feature_matrix": tf.VarLenFeature(tf.float32),
        "trans_matrix": tf.VarLenFeature(tf.float64),
    ...
    def _decode_record(record, name_to_features):
        ...
        for name in list(example.keys()):
            ...
            if t.dtype == tf.float32:
                ...
                t = tf.reshape(t, [seq_length, -1])
            if t.dype == tf.float64:
                t = tf.sparse_tensor_to_dense(t)
...
def create_model(bert_config, is_training, input_ids, input_mask,
                 ...
                 ntokens, label_ids_word_level, adjacent_matrix, syntactic_feature_matrix,
                 trans_matrix, dropout_rate=1.0, lstm_size=128, cell='lstm', num_layers=1):
    ...
    gat_bilstm_crf = GAT_BILSTM_CRF(embedded_chars=output_layer, hidden_unit=lstm_size, cell_type=cell, num_layers=num_layers,
                          ...
                          adjacent_matrix=adjacent_matrix, syntactic_feature_matrix=syntactic_feature_matrix, 
                          label_ids_word_level=label_ids_word_level, ntokens=ntokens, trans_matrix= trans_matrix)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        label_ids_word_level = features["label_ids_word_level"]
        adjacent_matrix = features["adjacent_matrix"]
        ...
        total_loss, logits, trans, predictsDict = create_model(
            ...
            num_labels, False, ntokens, label_ids_word_level,
            adjacent_matrix, syntactic_feature_matrix)
