...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode, use_pos, use_dependency):
    ...
    if use_pos == True and use_dependency == True:
        bias_matrix, syntactic_feature_matrix = matrix_generation_duplicate_low_dim_pos_dependency(sentence, ntokens, max_seq_length)
    elif use_pos == True and use_dependency == False:
        bias_matrix, syntactic_feature_matrix = matrix_generation_duplicate_low_dim_pos(sentence, ntokens, max_seq_length)
    elif use_pos == False and use_dependency == True:
        bias_matrix, syntactic_feature_matrix = matrix_generation_duplicate_low_dim_dependency(sentence, ntokens, max_seq_length)
    elif use_pos == False and use_dependency == False:
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            train_op = optimization.create_optimizer(
                total_loss, learning_rate, num_train_steps, num_warmup_steps, use_tpu, freeze_bert_parameters)
    ...
    if FLAGS.do_predict:
        ...
        if FLAGS.preprocessed_data_dir == None:
            ...
            filed_based_convert_examples_to_features(predict_examples, label_list,
                                                    ...
                                                    predict_file, use_pos=FLAGS.use_pos, use_dependency=FLAGS.use_dependency, mode="test")
        else:
